...
def main(args=None):
    ...
    output_root = os.path.join('./outputs_FMB', opt['RESUME_FROM'].split('/')[-1].split('.')[0], "Vis_IR_Weighted_Score_Fuse")
    ...
    for image_name in image_names:
        with torch.no_grad():
            ...
            seg_fuse = torch.max(seg1, dim=0)[0]*seg1 + torch.max(seg2, dim=0)[0]*seg2
