...
def main(args=None):
    ...
    output_root = os.path.join('./outputs_FMB', opt['RESUME_FROM'].split('/')[-1].split('.')[0], "Vis_IR_Ent_Select")
    ...
    for image_name in image_names:
        with torch.no_grad():
            ...
            seg1 = F.softmax(seg1, dim=0)
            seg2 = F.softmax(seg2, dim=0)
            ent1 = prob_2_entropy(seg1)
            ent1 = ent1.sum(dim=0)
            ent2 = prob_2_entropy(seg2)
            ent2 = ent2.sum(dim=0)
