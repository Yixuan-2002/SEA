...
def main(args=None):
    ...
    with torch.no_grad():
        ...
        for idx, mask in enumerate(grd_mask):
            demo = visual.draw_binary_mask(mask, color=random_color(rgb=True, maximum=1).astype(int).tolist(), text=text[idx], alpha=0.3)
